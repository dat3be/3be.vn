<?php
include_once(__DIR__.'/../KHANGAPI.php');
session_start();
$ketnoi = @mysqli_connect('localhost',$USERNAME,$PASSWORD,$DATABASE) or die("Error => DATABASE");
...
{
    $xss_patterns = ["<script", "javascript:", "onerror=", "onload=", "alert(", "document.cookie", "document.write", "<iframe", "<img", "src=", "eval("];
...
function banquyen($data) {
 if ($data == "Active") {
$show = '<span type="span" class="badge badge-light-success fs-6">Hoạt Động</span>';
}else if($data == "Invalid") {
$show = '<span type="span" class="badge badge-light-danger fs-6">Bị Khoá</span>';
} else {
$show = '<span type="span" class="badge badge-light-danger fs-6">Khác</span>';
}
return $show;
...
function status($data) {
    ...
    $show = '<span type="span" class="badge badge-light-danger fs-6">Đang Chờ Kích Hoạt.</span>';
    } else {
    $show = '<span type="span" class="badge badge-light-warning fs-6">Khác</span>';
    }
    return $show;
